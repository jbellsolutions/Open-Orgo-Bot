...
Open Orgo Bot has an Ubuntu 24.04 LTS x86_64 desktop beta. The Electron package embeds the harness server, so
...
- The native Electron window and embedded Open Orgo Bot server on GNOME Xorg and GNOME Wayland.
...
progress in [issue #29](https://github.com/jbellsolutions/open-orgo-bot/issues/29) and the safety hold in
[issue #345](https://github.com/jbellsolutions/open-orgo-bot/issues/345). Bundled
CUA supply-chain work is tracked in [issue #113](https://github.com/jbellsolutions/open-orgo-bot/issues/113). Xorg is tracked in
[issue #79](https://github.com/jbellsolutions/open-orgo-bot/issues/79), and guarded GNOME/Wayland support in
[issue #109](https://github.com/jbellsolutions/open-orgo-bot/issues/109).

## Download packages

Choose one Ubuntu 24.04 x86_64 package from the latest release:

- [Debian package (`Open-Orgo-Bot-amd64.deb`)](https://github.com/jbellsolutions/open-orgo-bot/releases/latest/download/Open-Orgo-Bot-amd64.deb) — recommended; APT installs its desktop dependencies and configures the bundled bot browser's sandbox.
- [Portable AppImage (`Open-Orgo-Bot.AppImage`)](https://github.com/jbellsolutions/open-orgo-bot/releases/latest/download/Open-Orgo-Bot.AppImage) — does not install system files.
- [SHA-256 checksums](https://github.com/jbellsolutions/open-orgo-bot/releases/latest/download/SHA256SUMS-ubuntu-x64.txt)

Versioned packages and previous releases remain available on the
[releases page](https://github.com/jbellsolutions/open-orgo-bot/releases).
...
- `release/Open Orgo Bot-<version>-amd64.deb`
- `release/Open Orgo Bot-<version>-x86_64.AppImage`
...
```sh
sudo apt install ./Open Orgo Bot-amd64.deb
```

Then open **Open Orgo Bot** from the GNOME application launcher. To remove it:
...
```sh
chmod +x release/Open Orgo Bot-*-x86_64.AppImage
./release/Open Orgo Bot-*-x86_64.AppImage
```

For a downloaded release AppImage, use `Open Orgo Bot.AppImage` in place of the versioned path above.
...
Applications launched from GNOME do not inherit the same interactive shell `PATH` as a terminal. Open Orgo Bot
...
```sh
OMB_EXTRA_PATH=/your/custom/bin ./release/Open Orgo Bot-*-x86_64.AppImage
```

Restart Open Orgo Bot after installing or signing in to a CLI.
...
24.04 GNOME Wayland session. Open Orgo Bot detects Wayland before XWayland when both `WAYLAND_DISPLAY` and
...
automatically. Open Orgo Bot does not capture screen audio, remember the selected monitor after restart, or
...
starts with `--no-overlay`, so Cua's decorative full-screen X11 cursor surface is never created. Open Orgo Bot also
...
The Xorg runtime uses private sockets, standard permission mode, per-action Open Orgo Bot approvals,
...
Run the CLI directly in a terminal, finish its sign-in flow, then restart Open Orgo Bot. If it lives outside the
...
```sh
chmod +x Open Orgo Bot-*-x86_64.AppImage
file Open Orgo Bot-*-x86_64.AppImage
```
