...
Every Open Orgo Bot desktop build can play either role:
...
The HTTPS address uses the host's managed outbound tunnel. TLS is verified by the operating system, and the client does not need Tailscale. HTTPS is intentionally restricted to Open Orgo Bot-managed companion names so a typo cannot redirect a paired-device token to an unrelated site.
