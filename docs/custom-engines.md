...
Two zero-code ways to run Open Orgo Bot bots on an engine the app doesn't ship.
...
- **Sign in first.** The driver has no auth flow of its own — run the CLI once
  in a terminal and log in there; Open Orgo Bot spawns it with your login intact.
