...
iPhone or iPad simulator, then run the `Open Orgo BotCompanion` scheme's UI tests
against that explicit simulator ID. For example, from `ios/`:

```sh
xcodebuild -project Open Orgo BotCompanion.xcodeproj -scheme Open Orgo BotCompanion \
