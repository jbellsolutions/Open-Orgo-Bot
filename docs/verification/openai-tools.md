...
use the harness's existing ownership and permission gates.
...
The probe checks the bot-owned or inherited team Orgo computer without
provisioning or waking it. Dispatch repeats the check so a removed key or
unavailable Orgo computer fails the run before model execution. Explicit Cloud
still permits creating/waking the bot’s own Orgo computer; a missing assigned
team computer requires explicit repair.

Run `pnpm exec vitest run server/routine-requests.test.ts` for target
selection and readiness. (Upstream's Box bridge fixture,
`openai-box.e2e`, is not part of Open Orgo Bot.)
