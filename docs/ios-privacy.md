# Open Orgo BotMobile privacy

Open Orgo BotMobile is a companion for an Open Orgo Bot service chosen and operated
by the user. Local Wi-Fi and Tailscale connections work without an Open Orgo Bot
...
- The computer remains the source of bots, transcripts, approvals, credentials,
  SQLite data, and screen images. Open Orgo Bot's hosted control plane does not
...
- When a user completes a supported credential request on the phone, the value
  ...
  written to
  iOS preferences or Keychain by Open Orgo BotMobile, chat, SQLite, logs, or the
...
- If the desktop user enables optional hosted access, Open Orgo Bot stores the
...
- The optional HTTPS route is proxied by Cloudflare to an outbound-only
  ...
  but are not written to the Open Orgo Bot control-plane database. Cloudflare may
  process IP addresses and connection/request metadata as Open Orgo Bot's service
...
- Connector tokens stay in the desktop operating system's encrypted credential
  ...
  database.
- When a user explicitly chooses Open Orgo Bot from another app's Share sheet, or
  ...
  Share sheet session removes the abandoned copy immediately; an Open Orgo BotMobile
...
Revoking the phone in Open Orgo Bot's Companion settings invalidates that device
credential. Transcript deletion is controlled by the Open Orgo Bot installation
...
[Open Orgo Bot Support](https://github.com/jbellsolutions/open-orgo-bot/issues) request
...
[Open Orgo Bot Support](https://github.com/jbellsolutions/open-orgo-bot/issues).
