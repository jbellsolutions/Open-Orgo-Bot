...
Open Orgo Bot saves a new server switched off. Use **Test** to start it briefly,
...
`OMB_CLAUDE_INHERIT_USER_CONFIG=1` on the Open Orgo Bot process; it restores
...
session's compaction window with `--autocompact` (2.1.122+). Open Orgo Bot reads
...
If you edit the file by hand, restart Open Orgo Bot. Every bot whose engine can
...
- **Testing is bounded.** The test command is stopped after the handshake (or
  ...
  It inherits none of Open Orgo Bot's workspace or provider credentials; only
