...
OpenCode is an optional Open Orgo Bot engine. Open Orgo Bot runs the maintained
...
2. Connect the providers you want in the OpenCode app, or run
   `opencode auth login`.
3. Restart Open Orgo Bot. It reuses OpenCode's existing connections and model
...
Open Orgo Bot does not copy or rewrite `auth.json`. The OpenCode CLI remains the
owner of provider authentication, and the same Zen or Go connection used by
the OpenCode desktop/TUI is used by Open Orgo Bot.
