...
Add OpenCode Go as a first-class Open Orgo Bot engine by running the maintained
OpenCode CLI over its official Agent Client Protocol (ACP) interface and selecting
models from the `opencode-go/*` provider.

The first implementation should reuse Open Orgo Bot's ACP runtime rather than call
...
provide inference; Open Orgo Bot does not currently have a provider-independent
...
- OpenCode accepts an `OPENCODE_API_KEY` environment variable. Open Orgo Bot should
...
the process lifecycle Open Orgo Bot uses for Grok and Gemini.
...
- refresh on the existing instance re-probe path so upstream additions do not
  require restarting Open Orgo Bot.
...
- load that session on the next Open Orgo Bot turn;
...
- concurrent Open Orgo Bot threads without shared ACP or credential state.
...
- OpenCode Go is shown as unavailable until both its CLI and credential are
  present; Open Orgo Bot never selects it as a runnable default otherwise.
- The setup flow works without restarting Open Orgo Bot and never silently executes
  an installer.
...
6. Can the existing ACP usage metadata distinguish cumulative session totals from
   per-turn usage so Open Orgo Bot does not double-count it?
