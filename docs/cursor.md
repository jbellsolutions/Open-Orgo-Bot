...
Cursor is an optional Open Orgo Bot engine. Open Orgo Bot runs the official
...
3. Confirm `cursor-agent --version` works. The binary installs to `~/.local/bin` by
   default; Open Orgo Bot already looks there when launched from a GUI.
...
CLI also implements ACP `session/set_model`, Open Orgo Bot pins the same id over
...
does not provide a bot approval level. Open Orgo Bot app turns always provide
one: both **Ask for approval** and **Approve for me** launch Cursor without
`--force`, then Open Orgo Bot handles its permission requests according to the
bot's current level.
