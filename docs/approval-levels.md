...
provider's own permission modes, passed through. Open Orgo Bot does not judge an
...
| **Custom (`config.toml`)** | Codex only. Open Orgo Bot reads and reapplies the effective approval and sandbox settings from your Codex configuration. |
...
ends. Open Orgo Bot keeps no standing grant for a provider's tool. It is not
...
Native modes require a CLI version that supports them; Open Orgo Bot does not
silently substitute unrestricted access when a mode is rejected.

### Read-only integration tools

Open Orgo Bot's built-in agents MCP server describes nine scoped reads with
...
provider's session. Its quieter default is Full access; Open Orgo Bot keeps its
