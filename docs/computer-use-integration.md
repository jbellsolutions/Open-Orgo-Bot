# Computer use & browser use in Open Orgo Bot

Decision doc, 2026-08-12. How bots in Open Orgo Bot get local computer use and
...
supply-chain contract tracked in [#113](https://github.com/jbellsolutions/open-orgo-bot/issues/113): pinned archive
...
receives the validated MCP proxy contract. Xorg is tracked in [#79](https://github.com/jbellsolutions/open-orgo-bot/issues/79);
GNOME/Wayland additionally requires WinRects v8 plus the exact Cua health-report contract tracked in
[#109](https://github.com/jbellsolutions/open-orgo-bot/issues/109).
...
1. **Spawn from the Electron main process, never from the server/gateway
   ...
   Spawned from Electron main → the grant is Open Orgo Bot's, users see ONE
   prompt named Open Orgo Bot, and the bundled driver inherits it. Spawned from
...
- Ship the binary at `Open Orgo Bot.app/Contents/Resources/cua-driver`,
