...
(enabled ? it : it.skip)("edits and arranges the team canvas without losing bots or conversations", async () => {
  ...
  try {
    ...
    // The full-app fixture has no Orgo credentials. The explicit + entry
    ...
    await expect.poll(snapshot).toContain("Connect your Orgo account before creating a cloud computer.");
    await expect.poll(snapshot).toContain("Your Orgo plan and usage charges apply.");
    const inputRefs = (await ui("snapshot")).refs as Record<string, { role: string; name: string }>;
    const nameInput = Object.entries(inputRefs).filter(([, entry]) => entry.role === "textbox" && entry.name === "New Orgo computer");
    ...
    receipts.unconfiguredComputerCreation = "Explicit Add entry shows cost disclosure and keeps Create Orgo disabled without credentials.";
