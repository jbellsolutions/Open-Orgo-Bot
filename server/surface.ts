...
export function surfacePrompt(
  ...
  if (text) text += RESTATE_SENTENCE + (opts.canSelect
    ? SURFACE_AUTHORITY.replace("explain the mismatch and ask the user to change the conversation's computer selector", "inspect connected choices with select_computer and select the requested available place; on a pending result end this turn so Open Orgo Bot can reconnect the correct tools and continue the original request")
    : SURFACE_AUTHORITY);
  else if (!opts.note && !opts.canSelect) {
    text = " No computer or built-in browser tools are mounted this turn. You cannot open apps, click, or inspect a screen through Open Orgo Bot. If asked for screen work, explain this and ask the user to choose and connect a computer in the Computer panel; do not claim to have opened or checked it.";
  }
  if (opts.pinned) {
