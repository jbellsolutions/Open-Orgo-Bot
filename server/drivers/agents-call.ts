...
function routineFields(args: Json): { fields: Json; error?: string } {
  ...
  // "box" is upstream's name for the same explicit cloud destination.
  const destination = (value: unknown) => value === "orgo" || value === "box" ? "cloud" : value;
  ...
  if (runOn != null && runOn !== "maus" && runOn !== "cloud") {
    return { fields, error: 'Use run_on="maus" for the bot’s current model and configured computer (including VPS), or run_on="orgo" only for the bot’s Orgo cloud computer. Legacy "cloud" also means Orgo.' };
  }
