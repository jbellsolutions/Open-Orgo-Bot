...
describe("structured tool execution boundaries", () => {
  ...
  it("redacts the per-turn Orgo computer API key and control token from published text", async () => {
    const orgoKey = "sk_orgo_fixture_canary_0123456789";
    const controlToken = "orgo-control-fixture-canary-9876";
    const f = await fixture((_request, response) => sse(response, [
      chunk({ content: `Key ${orgoKey} and ${controlToken}.` }, "stop"),
    ]), "grok");
    await f.start({ integrations: { computer: {
      kind: "orgo", computerId: "11111111-1111-4111-8111-111111111111", apiKey: orgoKey,
      control: { url: "http://127.0.0.1:9/control", token: controlToken },
    } } });
    expect(await f.completed()).toMatchObject({ ok: true });
    const events = JSON.stringify(f.recorder.events);
    expect(events).toContain("[redacted]");
    expect(events).not.toContain(orgoKey);
    expect(events).not.toContain(controlToken);
  });
