...
const ROUTINE_FIELDS_SCHEMA = {
  ...
  run_on: {
    type: "string",
    enum: ["maus", "orgo"],
    description: "Default maus keeps the bot's selected model and configured computer, INCLUDING a self-hosted VPS. Omit this field for normal schedules. orgo explicitly runs the routine on the bot's Orgo cloud computer; it requires Orgo setup and is not the generic cloud/VPS option. Legacy cloud values from list_routines mean orgo, not VPS.",
  },
  timeout_minutes: {
    type: "integer",
    minimum: 5,
