...
const computerLabels: Record<ComputerKind, string> = { thisComputer: "this computer", localVm: "local virtual machines", box: "Orgo cloud computers", vps: "VPS computers" };
...
export function computerKindForResource(resource: string): ComputerKind | undefined {
  ...
  // Open Orgo Bot: the policy schema keeps upstream's "box" key for the
  // cloud-computer switch; Orgo computers are the fork's cloud computers, so
  // their claims are governed by it (turning it off must refuse them).
  if (resource.startsWith("computer:orgo:") || resource.startsWith("computer:orgo-bot:")) return "box";
