...
beforeAll(async () => {
  ...
  writeFileSync(join(data, "config.json"), JSON.stringify({ instances: {
    ...
    // Cursor CLI makes every provider probe slow. Missing CLIs keep both out of
    // the default selection so every bot runs the fake Claude.
    hermes: { driver: "hermesAgent", config: { cli: join(fixtureHome, "missing-hermes") } },
    cursor: { driver: "cursorAgent", config: { cli: join(fixtureHome, "missing-cursor") } },
  } }));
