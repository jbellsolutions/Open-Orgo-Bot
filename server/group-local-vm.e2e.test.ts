...
describe("Group Local VM ownership on the real isolated server", () => {
  ...
  it.skip("holds a cloud turn for a marked output question and resumes after the person's reply", async () => {
  ...
  it.skip("does not stall an admitted turn waiting for another turn's shared computer", async () => {
  ...
  it.skipIf(process.platform === "linux")("carries the recorded macOS permission failure into the failed turn", async () => {
    ...
    try {
      ...
      if (process.platform === "darwin") expect(JSON.stringify(state)).toContain("Relaunch Open Orgo Bot after granting the missing macOS permission");
      await idle(bots[0].id);
      expect(existsSync(dumpFile)).toBe(false);
    } finally {
      rmSync(cuaDescriptor, { force: true });
  ...
  // Upstream Box runner case; Open Orgo Bot has no Box engine (see above).
  it.skip("runs a channel speaker's own Cloud destination on its Box, waking it first", async () => {
    const { bots, group } = await room();
    try {
      await api("PUT", "/api/config", { box: { token: "box_fixture" } });
