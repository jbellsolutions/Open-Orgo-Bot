...
export async function runPhoneSetup(
  ...
  const signIn = async (): Promise<boolean> => {
    try {
      if (await deps.accountReady?.(options)) {
        io.log("Your saved Open Orgo Bot account can be reused. The connection will be checked when the server starts.");
        return true;
      }
      io.log("Sign in to an Open Orgo Bot account using an emailed code. This is separate from your AI provider account.");
  ...
  for (;;) {
    const device = await io.choose("Use Open Orgo Bot on your phone?", [
      "Skip for now",
    ...
    while (!back) {
      ...
      if (route === 0) {
        ...
        io.log("Starting it may download the Cloudflare connector. It stays active while Open Orgo Bot runs; stop Open Orgo Bot to close the connection.");
      ...
      if (route === 1) {
        ...
        if (!await io.confirm("Allow Open Orgo Bot to serve HTTPS to your tailnet while it runs?", false)) continue;
...
export function phonePairingInstructions(
  ...
  if (!input.ready || !origin) {
    return ["Phone access is not ready yet. No phone QR should be shown until the HTTPS connection is verified.",
      "Your local Open Orgo Bot can still be used on this computer."];
  }
  return [
    phone === "ios"
      ? "On iPhone or iPad, scan the QR with Camera to open Safari. If you already have the Open Orgo Bot iOS app, use its pairing scanner or paste the full pairing link there."
      : "On Android, open the Open Orgo Bot app and scan the QR with its pairing scanner. The QR is an app link, so Camera will not open it in a browser.",
