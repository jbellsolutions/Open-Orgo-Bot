...
/** Hermes is Open Orgo Bot's default engine; Claude is the next choice. */
const hermesFirst = (available: readonly SelectableInstance[]) =>
...
export function selectDefaultModelSelection(
  ...
  if (company && instances.some((instance) => company(instance.instanceId))) {
    // Enrolled: an installed but signed-out personal engine would send every
    ...
    pick = hermesFirst(ready.filter((instance) => !company(instance.instanceId))) ?? hermesFirst(ready) ?? hermesFirst(available);
  } else {
    pick = hermesFirst(available);
  }
