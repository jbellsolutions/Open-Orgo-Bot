...
/** Names used to route Open Orgo Bot's built-in MCP proxies and their ephemeral
...
export function isHarnessOwnedMcpEnvName(name: string): boolean {
  return (
    ...
    name.startsWith("OMB_") ||
    name.startsWith("OGB_") ||
    name.startsWith("OOB_")
  );
}
...
function environmentNameError(name: string): string | null {
  ...
  if (isHarnessOwnedMcpEnvName(name)) {
    return `Environment variable “${name}” is reserved by Open Orgo Bot.`;
  }
  return null;
}
...
export function mcpServerNameError(name: string): string | null {
  ...
  if (RESERVED_MCP_NAMES.has(name)) return "That name is reserved by Open Orgo Bot.";
  return null;
}
