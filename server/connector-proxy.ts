...
// into first-class Open Orgo Bot chat cards. The agent never authors an auth
...
async function handle(message: Json): Promise<void> {
  ...
  if (method === "tools/call") {
    ...
    if (requests.length) {
      ...
      send(textResult(
        id,
        `Open Orgo Bot showed the user a secure connection card for ${labels}. End this turn now. The app will continue the task automatically after the connection finishes.`,
      ));
      return;
    }
    if (/WAIT_FOR_CONNECTIONS$/i.test(name)) {
      send(textResult(id, "Open Orgo Bot is handling connection completion and will continue the task automatically."));
      return;
    }
  }
  try {
