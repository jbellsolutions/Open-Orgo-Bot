...
Open **Connected apps** from the sidebar, choose a service, and complete its authorization in your normal browser. Open Orgo Bot turns the resulting connection into tools that supported agent engines can use.

<ProductScreenshot name="docs-connected-apps" alt="The clean Open Orgo Bot connected-app marketplace" caption="Search the marketplace or review every account from the Connected tab." />

## Official packaged app

The signed desktop app registers its own installation with Open Orgo Bot's managed connected-apps service. You do **not** need to find or paste a Composio key.
...
The official managed Open Orgo Bot service keeps X disabled until it owns an appropriate X OAuth app. Older Open Orgo Bot tasks that refer to the connector as `x` continue to work; Open Orgo Bot translates that alias to Composio's `twitter` toolkit automatically.

## Multiple accounts

Connect more than one account for the same toolkit and give each a clear alias such as `work`, `personal`, or `client-acme`. When several accounts could perform an action, Open Orgo Bot requires explicit selection instead of silently replacing the first connection.
