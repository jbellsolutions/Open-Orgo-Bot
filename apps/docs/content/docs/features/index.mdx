...
description: A map of the capabilities currently available in Open Orgo Bot.
icon: Sparkles
---

Open Orgo Bot turns agent CLIs into a messaging-style workspace. The harness normalizes each provider into one event stream, so the interface can treat different agents consistently while preserving their native strengths.
