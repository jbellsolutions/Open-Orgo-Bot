...
if [ -n "${OPENMAUSBOT_POSTINSTALL_TEST_ROOT:-}" ]; then
  ...
  case "$TEST_ROOT" in
    /tmp/*) ;;
    *) echo "Open Orgo Bot test install root must stay under /tmp" >&2; exit 1 ;;
  esac
...
if [ "$TEST_MODE" -eq 0 ]; then
  if command -v update-alternatives >/dev/null 2>&1; then
    update-alternatives --remove open-orgo-bot "/opt/Open Orgo Bot/open-orgo-bot"
  elif [ -L /usr/bin/open-orgo-bot ] && [ "$(readlink /usr/bin/open-orgo-bot)" = "/opt/Open Orgo Bot/open-orgo-bot" ]; then
    rm -- /usr/bin/open-orgo-bot
  fi
fi
...
if [ -L "$APPARMOR_DIR" ] || [ -L "$profile" ] || [ ! -f "$profile" ]; then
  echo "Open Orgo Bot AppArmor profile is unsafe; refusing to remove it: $profile" >&2
  exit 1
fi
if [ "$TEST_MODE" -eq 0 ] && [ -x /usr/bin/ischroot ] && /usr/bin/ischroot; then
  ...
  if ! "$APPARMOR_PARSER" -R "$profile"; then
    echo "Open Orgo Bot could not unload its browser AppArmor profile: $profile" >&2
    exit 1
  fi
elif [ -r "$APPARMOR_PROFILES" ]; then
  echo "Open Orgo Bot needs apparmor_parser to unload its browser profile: $profile" >&2
  exit 1
fi
rm -- "$profile"
