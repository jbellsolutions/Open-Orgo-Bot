...
function LocalVmPane({
  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      if (!bridge) {
        setError("The two-desktop workspace requires the Open Orgo Bot desktop app.");
        return;
      }
      try {
        ...
        setError(
          cause instanceof Error && cause.message === "layout-unavailable"
            ...
            : "Open Orgo Bot could not connect this Local VM viewer.",
        );
      }
    };
  ...
  const updateLayout = useCallback(() => {
    ...
    void bridge
      ...
      .catch(() => setError("Open Orgo Bot could not position this Local VM viewer."));
...
export function LocalVmWorkspace({
  ...
  const handBack = useCallback(async () => {
    ...
    try {
      ...
      setControlError("Open Orgo Bot could not hand control back. The workspace stayed open.");
      return false;
    } finally {
