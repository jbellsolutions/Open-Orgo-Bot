...
export const NEWER_PACKAGE_MESSAGE = "This file was made by a newer Open Orgo Bot. Update the app, then import it again.";
...
export function downgradeToV1(document: PackageDocument):
  ...
  if (!pkg.agents.length || !pkg.team) return { error: "Packages without bots cannot be opened by older versions of Open Orgo Bot." };
  const referenced = new Set(pkg.agents.flatMap((agent) => agent.skills ?? []));
  const skills = (pkg.skills?.entries ?? []).filter((skill) => referenced.has(skill.name));
  if (skills.length > PACKAGE_V1_MAX_SKILLS) {
    return { error: `Older versions of Open Orgo Bot accept at most ${PACKAGE_V1_MAX_SKILLS} skills in a package.` };
  }
  const crowded = pkg.agents.find((agent) => (agent.skills?.length ?? 0) > PACKAGE_V1_MAX_SKILLS);
  if (crowded) return { error: `Older versions of Open Orgo Bot accept at most ${PACKAGE_V1_MAX_SKILLS} skills per bot (${crowded.key} has more).` };
  ...
  if (outOfRange) return { error: `Older versions of Open Orgo Bot accept run limits from 5 to 240 minutes (${outOfRange.key} has ${outOfRange.timeoutMinutes}).` };
  ...
  try {
    ...
    return { error: error instanceof Error ? error.message : "This package cannot be written for older versions of Open Orgo Bot." };
  }
}
