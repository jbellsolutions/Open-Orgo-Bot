...
test("the shipped installer replaces the AppImage and queues its original path for relaunch", (t) => {
  ...
  const launched = join(workspace, "Open Orgo Bot-0.1.43-x86_64.AppImage");
  const staged = join(workspace, "pending", "Open Orgo Bot-0.1.44-x86_64.AppImage");
  ...
  assert.deepEqual(readdirSync(workspace).sort(), ["Open Orgo Bot-0.1.43-x86_64.AppImage", "data", "pending"]);
...
test("a rejected AppImage relaunch is reported without quitting the desktop", (t) => {
  ...
  const launched = join(workspace, "Open Orgo Bot.AppImage");
...
test("Electron relaunch waits for deferred cleanup and the replacement acquires the same data directory", {
  ...
  t.after(() => rmSync(workspace, { recursive: true, force: true }));
  const launched = join(workspace, "Open Orgo Bot-1.0.0.AppImage");
  const staged = join(workspace, "Open Orgo Bot-2.0.0.AppImage");
