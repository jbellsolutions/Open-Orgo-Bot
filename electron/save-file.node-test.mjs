...
before(() => {
  ...
  botHome = path.join(home, ".openorgobot");
...
describe("save-file path validation", () => {
  ...
  it("accepts a file under a symlinked bot home", { skip: !canSymlink }, async () => {
    ...
    fs.symlinkSync(realBotHome, path.join(linkedHome, ".openorgobot"));

    const viaLink = path.join(linkedHome, ".openorgobot", "report.docx");
