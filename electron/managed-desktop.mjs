...
// "box" is upstream's policy key for the cloud-computer switch; in Open Orgo
// Bot it governs Orgo cloud computers (see server/managed-policy.ts).
...
export function createManagedDesktopClient({ store, applyConnection, applyPolicy = async () => {}, migrateIdentity = async () => {}, openBrowser, platform, deviceName, appVersion,
  ...
  function clearConnection() {
    ...
    const operation = (async () => {
      ...
      const warnings = [];
      if (runtime.status === "rejected") warnings.push("The local runtime did not confirm stopping Company tasks. Quit and reopen Open Orgo Bot before using Company models again.");
  ...
  async function endAccess(stamp, message) {
    ...
    catch { message += " Quit and reopen Open Orgo Bot to confirm Company tasks have stopped."; }
  ...
  return {
    ...
    async start() {
      ...
      catch { markRestored(); return current(stamp) ? publish({ status: "unavailable", message: "Company sign-in could not be restored. Unlock your system keychain and restart Open Orgo Bot." }) : snapshot(); }
