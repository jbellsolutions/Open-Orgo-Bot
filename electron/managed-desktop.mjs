...
export function createManagedDesktopClient({ store, applyConnection, openBrowser, platform, deviceName,
  ...
  return {
    ...
    async start() {
      ...
      catch { return current(stamp) ? publish({ status: "unavailable", message: "Company sign-in could not be restored. Unlock your system keychain and restart Open Orgo Bot." }) : snapshot(); }
      return refresh();
    },
    async begin(input) {
